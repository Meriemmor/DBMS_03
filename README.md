...
>
> <img width="906" height="319" alt="image" src="https://github.com/user-attachments/assets/97ab027f-6f75-4d92-9858-529a67252a6c" />
...
> In a relational table, each cell is supposed to contain just one value. If you try to store multiple author_ids in a single column of a book table, things won´t make sense anymore. It makes index very difficult which results in the database not going through queries properly.This means that even simple tasks like adding or removing an author from a book turn into text updates instead of row operations. In order to avoid this,you can use a seperate join table so that every author-book relationship gets its own row with proper foreign keys pointing to both the author and the book. That way the data is clean and queries become faster and simpler.
...
> key.
>
> If the same member borrows the same copy more than once on the same day, the triple (member_no, copy_no, loan_date) would repeat, violating uniqueness. A surrogate key like loan_id avoids this completely. It’s just a unique ID generated by the system, so every loan record gets its own identifier .
---
...
>
> <img width="557" height="52" alt="image" src="https://github.com/user-attachments/assets/566e0a14-3e7d-4dd8-b80d-0318a312f22b" />
...
The delete won’t go through. If an author is still linked in the writes table, SQLite will block the operation and show an error. So before deleting the author, the librarian has to first remove all related rows from writes.
...
It’s a candidate key, meaning it can uniquely identify each member and could have been used as the primary key. In this case, though, member_no was chosen instead.
...
> at runtime?
 ...
 The first insert goes through without any complaint—even though member 9999 and copy 9999 don’t exist—because foreign key checks are turned off (`PRAGMA foreign_keys = OFF`). So SQLite just accepts the data without verifying it.

The second insert, however, fails with a *FOREIGN KEY constraint failed* error. That’s because foreign key enforcement is now turned on.

The key point is simply defining foreign keys in your table isn’t enough in SQLite. They only actually work if you explicitly enable them at runtime with `PRAGMA foreign_keys = ON`.
...
```sql
1|978-0-201-96426-4|A-01
2|978-0-201-96426-4|A-02
```
...
SQL:
```sql
978-0-072-46563-1|B-07
978-0-13-110362-7|C-12
```
